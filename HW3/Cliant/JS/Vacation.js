let port = "7011";
let server = `https://localhost:${port}`;
if (location.hostname === "localhost" || location.hostname === "127.0.0.1") {
    console.log(server);
}
else {
    server = "https://proj.ruppin.ac.il/cgroup61/test2/tar1/";
}
const api = server + "api/Vacations";
...
function getSCB(Vacations) {
    ...
    for (let i = 0; i < Vacations.length; i++) {
        if (Vacations[i].flatId == flatId) {
            ...
            VacationString += `<p class='card-text'><strong>User ID:</strong> ${Vacations[i].userId}</p>`;
            VacationString += `<p class='card-text'><strong>Flat ID:</strong> ${Vacations[i].flatId}</p>`;
            VacationString += `<p class='card-text'><strong>Start date:</strong> ${Vacations[i].startDate}</p>`;
            VacationString += `<p class='card-text'><strong>End date:</strong> ${Vacations[i].endDate}</p>`;
            VacationString += `</div>`
            VacationString += `</div>`
            VacationString += `</div>`
        }
